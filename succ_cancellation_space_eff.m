...
snr_dB = 4:0.5:8;   % SNR range in dB
...
for idx = 1:length(snr_dB)
    ...
    for trial = 1:num_trials
        ...
        [decoded_word, L, C] = SCDecode(noise_variance, n, frozen_bits,received_signal);
...
title('(Space Efficient) BLER vs SNR for BPSK with SC decoder in AWGN Channel');
legend('SC Decoding');


function [decoded_word,L,C] = SCDecode(noise_var, n, frozen_bits,y)
    ...
    C = zeros(m+1,n, 2); % Initialize decoded bits
    ...
    for phi = 0:n-1
        ...
        L = recursivelyCalcL(m, phi, L, m, C); % Calculate LLRs

        if frozen_bits(phi+1) % Check if u_hat is frozen
            C(m+1,1,mod(phi,2)+1) = 0; % Set to frozen value
        else
            ...
            % end
            if L(m+1, 1) > 0
                C(m+1,1,mod(phi,2)+1) = 0;
            else
                C(m+1,1,mod(phi,2)+1) = 1;
            end
        end

        if mod(phi, 2) == 1
            C = recursivelyUpdateC(m, phi, C, m);
        end
    end

    decoded_word = C(1,:,1); % Return the decoded word
    % dec_ub = B(end,:);
end


function C = recursivelyUpdateC(lambda, phi, C, m)
    if mod(phi, 2) == 0
        return;
    end
    ...
    for beta = 0:(2^(m-lambda))-1
        ...
        C(lambda-1+1,2*beta+1,mod(psi,2)+1) = mod(C(lambda+1,beta+1,1)+ C(lambda+1,beta+1,2),2);
        C(lambda-1+1,2*beta+1+1,mod(psi,2)+1) = C(lambda+1,beta+1,2);
    end

    if mod(psi, 2) == 1
        C = recursivelyUpdateC(lambda - 1, psi, C, m);
    end
end

function L = recursivelyCalcL(lambda, phi, L, m, C)
    if lambda == 0
        return; % Base case
    end
    ...
    psi = floor(phi / 2);
    if mod(phi, 2) == 0
        L = recursivelyCalcL(lambda - 1, psi, L, m, C);
    end

    for beta = 0:(2^(m-lambda))-1
        if mod(phi, 2) == 0
            % Apply Equation (1)
            L(lambda+1, beta+1) = log((custom_exp(L(lambda-1+1, 2*beta+1))*custom_exp(L(lambda-1+1, 2*beta+1+1))+1)...
                /(custom_exp(L(lambda-1+1, 2*beta+1))+ custom_exp(L(lambda-1+1, 2*beta+1+1))));
        else
            % Apply Equation (2)
            u_prime = C(lambda+1, beta+1, 1);
            L(lambda+1, beta+1) = log(custom_exp(L(lambda-1+1, 2*beta+1))^(1-2*u_prime)*custom_exp(L(lambda-1+1, 2*beta+1+1)));
